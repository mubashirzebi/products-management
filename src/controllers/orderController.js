...
const orderCreation = async (req, res) => {
  try {
    ...
    const searchCartDetails = await cartModel.findOne({ _id: cartId, userId });
    ...
    const totalQuantity = searchCartDetails.items.map((x) => x.quantity).reduce(reducer);
    ...
    await cartModel.findOneAndUpdate({ _id: cartId, userId }, {
      $set: {
        items: [],
        totalPrice: 0,
        totalItems: 0,
...
const updateOrder = async function (req, res) {
  try {
    ...
    const order = await orderModel.findOne({ _id: orderId, isDeleted: false, userId });
    ...
    if (status == 'completed' && order.status == 'pending') {
      await orderModel.findOneAndUpdate({ _id: orderId }, { $set: { status: 'completed' } });
